...
**Archived From:** docs/plans/2026-05-04-api-converter-plan/

---

## Original Goal

在 ZZ proxy 中引入显式的协议转换路由前缀（`/a2o/v1/*`、`/o2a/v1/*` 等），实现主流 LLM API 接口规范之间的双向转换，初期聚焦 Anthropic Claude API ↔ OpenAI Chat Completions API。保持 `/v1/*` 现有透明代理行为不变。

---

## Completed Scope (Initial Release)

- ✅ 透明代理 `/v1/*` 完全保持不变
- ✅ 新增 `/a2o/v1/*`（Anthropic → OpenAI Chat）
- ✅ 新增 `/o2a/v1/*`（OpenAI Chat → Anthropic）
...
- 实际的转换器遥测插桩（P9 仅实现基础设施，未连接到转换器）

---

## Remaining Items (建议移至 roadmap)

以下事项可作为后续迭代方向：

1. **转换器遥测插桩**：将 P9 实现的遥测基础设施连接到 P2/P3/P5 转换器中
2. **集成测试完善**：实现 phase-P8-verification.md §1 中的完整测试矩阵（当前为占位符）
3. **OpenAI Responses 支持**：实现 `/a2r/v1/*` 和 `/r2a/v1/*` 前缀
4. **显式格式路由**：实现 `/anthropic/v1/*`、`/openai/v1/*`、`/responses/v1/*`
5. **高级字段映射**：tool_use 复杂结构、多模态内容

---

## Phase Completion Status

- ✅ P0 — Spec & Field Mapping Freeze
- ✅ P1 — Skeleton
- ✅ P2 — Request a2o
- ✅ P3 — Response o2a (non-stream)
- ✅ P4 — Routing & Handler
- ✅ P5 — Stream SSE
- ✅ P6 — Fallback & Logging
- ✅ P7 — Config Extension
- ✅ P8 — Verification & Docs
- ✅ P9 — Iteration Telemetry (基础设施)
...
- ⚠️ cargo clippy --all-targets -- -D warnings: Pre-existing warnings in request_journal.rs (not related to converter work)
- ✅ Manual acceptance document created: docs/active-work/api-converter/manual-acceptance.md
- ✅ Developer guide created: docs/dev/api-converter.md
- ✅ README.md updated with protocol conversion section
...
- [Manual Acceptance](../../active-work/api-converter/manual-acceptance.md)
- [Developer Guide](../../dev/api-converter.md)

---

## Tech Stack

Rust, axum, hyper, tokio, serde_json, bytes, tracing, sha1, chrono
